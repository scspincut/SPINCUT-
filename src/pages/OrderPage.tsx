...
export default function OrderPage() {
  ...
  const total = CATALOG.reduce((s, item) => s + (quantities[item.ref] || 0) * item.price, 0)
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4 pb-36">
        ...
        <div className="space-y-2">
          {CATALOG.map(item => {
            ...
            return (
              <div
                key={item.ref}
                className={`rounded-xl p-4 border transition-all ${selected ? 'bg-[#1a1200] border-[#d4780f]/50' : 'bg-[#161616] border-[#1e1e1e]'}`}
              >
                <div className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <span className="font-mono text-[10px] text-[#666] bg-[#1e1e1e] px-1.5 py-0.5 rounded">{item.ref}</span>
                    <p className="text-white font-semibold text-sm mt-1">{item.designation} <span className="text-[#555]">{item.queue}</span></p>
                    {selected && (
        ...
        <p className="text-[#333] text-xs text-center">Prix HT — Tarifs réservés aux clients SPINCUT</p>
      </div>
