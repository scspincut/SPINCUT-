...
export default function BoutiquePage() {
  ...
  const diameters = useMemo(() => [...new Set(activeTabCatalog.map(p => p.diametre).filter(d => d && d !== '/'))].sort((a, b) => parseFloat(a) - parseFloat(b)), [activeTabCatalog])
  const lcValues  = useMemo(() => [...new Set(activeTabCatalog.map(p => p.lc).filter(l => l && l !== '/'))].sort((a, b) => parseFloat(a) - parseFloat(b)), [activeTabCatalog])
  const dentsValues = useMemo(() => [...new Set(activeTabCatalog.map(p => p.dents).filter(d => d && d !== '/'))].sort(), [activeTabCatalog])
  const hasPM = useMemo(() => activeTabCatalog.some(p => p.pm), [activeTabCatalog])

  const filtered = useMemo(() => activeTabCatalog.filter(p =>
    (filterDiam === null || p.diametre === filterDiam) &&
    (filterLC === null || p.lc === filterLC) &&
    (filterDents === null || p.dents === filterDents) &&
    (!filterPM || p.pm)
  ), [activeTabCatalog, filterDiam, filterLC, filterDents, filterPM])
    ...
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full pb-24">
        ...
        {homeView && (
          <>
            ...
            <div className="px-4 space-y-3 pt-5 pb-6">
              ...
              {/* Mes outils favoris — toujours visible */}
              <button
                onClick={enterFavs}
                className="w-full rounded-2xl relative overflow-hidden active:scale-[0.98] transition-all text-left"
              ...
              >
                <div className="absolute inset-0 flex items-center px-5">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-1 font-semibold" style={{ color: '#e03c3c' }}>
                      {favorites.size > 0 ? `${favorites.size} outil${favorites.size > 1 ? 's' : ''} sauvegardé${favorites.size > 1 ? 's' : ''}` : 'Aucun favori pour l\'instant'}
                    </p>
                  </div>
                  <svg width="30" height="30" fill={favorites.size > 0 ? '#e03c3c' : '#3a1a1a'} viewBox="0 0 24 24" style={{ flexShrink: 0, opacity: 0.9 }}>
        ...
        {!homeView && !favsView && (
          <>
            {/* Filter bar */}
            {!catalogLoading && !catalogError && activeTabCatalog.length > 0 && (
              <div className="px-4 pt-3 pb-1 flex items-center gap-2">
                ...
                <span className="text-xs text-[#444] flex-1">{filtered.length} produit{filtered.length !== 1 ? 's' : ''}</span>
                <button
            ...
            {/* Product list — directly, no category picker */}
                ...
                {activeTabCatalog.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap mb-1">
                          <span className="font-mono text-xs font-bold text-white bg-[#1e1e1e] px-2 py-0.5 rounded border border-[#2a2a2a]">{item.ref}</span>
                          <StockBadge stock={item.stock} />
